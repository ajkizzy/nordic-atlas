// Cuisine metadata: label + raw SVG markup (lucide-react icon paths, ISC licensed).
// Kept as raw markup so the same glyph can be used in React *and* injected
// into Leaflet divIcon HTML without pulling react-dom/server into the bundle.
import type { Cuisine } from '@/types/db';

export const CUISINE_META: Record<Cuisine, { label: string; svg: string }> = {
  burger: {
    label: 'Burger',
    svg: '<circle cx="12.5" cy="8.5" r="2.5"/><path d="M12.5 2a6.5 6.5 0 0 0-6.22 4.6c-1.1 3.13-.78 3.9-3.18 6.08A3 3 0 0 0 5 18c4 0 8.4-1.8 11.4-4.3A6.5 6.5 0 0 0 12.5 2Z"/><path d="m18.5 6 2.19 4.5a6.48 6.48 0 0 1 .31 2 6.49 6.49 0 0 1-2.6 5.2C15.4 20.2 11 22 7 22a3 3 0 0 1-2.68-1.66L2.4 16.5"/>',
  },
  bbq: {
    label: 'BBQ & Grill',
  ...
  bakery: {
    label: 'Bakery',
    svg: '<path d="m4.6 13.11 5.79-3.21c1.89-1.05 4.79 1.78 3.71 3.71l-3.22 5.81C8.8 23.16.79 15.23 4.6 13.11Z"/><path d="m10.5 9.5-1-2.29C9.2 6.48 8.8 6 8 6H4.5C2.79 6 2 6.5 2 8.5a7.71 7.71 0 0 0 2 4.83"/><path d="M8 6c0-1.55.24-4-2-4-2 0-2.5 2.17-2.5 4"/><path d="m14.5 13.5 2.29 1c.73.3 1.21.7 1.21 1.5v3.5c0 1.71-.5 2.5-2.5 2.5a7.71 7.71 0 0 1-4.83-2"/><path d="M18 16c1.55 0 4-.24 4 2 0 2-2.17 2.5-4 2.5"/>',
  },
  deli: {
    label: 'Deli & Smørrebrød',
...
export const CUISINE_ORDER = Object.keys(CUISINE_META) as Cuisine[];

/** Full <svg> element markup for a cuisine glyph, used in map pins. */
export function cuisineSvg(cuisine: Cuisine, size = 14, color = '#ffffff') {
  return (
    `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" ` +
    `fill="none" stroke="${color}" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round">` +
    `${CUISINE_META[cuisine].svg}</svg>`
  );
}
